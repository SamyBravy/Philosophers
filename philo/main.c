...
/*   Updated: 2024/07/03 22:27:17 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_mutexes(t_philo *philo)
{
	...
	i = 0;
	while (i < philo->nb_philo)
		if (pthread_mutex_init(&(philo->last_meals_mutex[i++]), NULL) != 0)
		{
			i = 0;
			while (i < philo->nb_philo)
				pthread_mutex_destroy(&(philo->forks[i++]));
...
void	*philosopher(void *philo)
{
	...
	while (((t_philo *)philo)->nb_eat == -1 || eaten++ < ((t_philo *)philo)->nb_eat)
	{
		...
		//	return (NULL);
		printf("\033[0;33m%ld %d is thinking\n\033[0m", get_time(((t_philo *)philo)->tv), i);
		...
		printf("\033[0;32m%ld %d is eating\n\033[0m", ((t_philo *)philo)->last_meals[i - 1], i);
...
int	main(int argc, char **argv)
{
	...
	while (i < philo.nb_philo)
	{
		if (pthread_create(&(philo_threads[i++]), NULL, &philosopher, &philo))
		{
			free(philo_threads);
			pthread_mutex_destroy(&(philo.mutex));
			j = 0;
	...
	while (i < philo.nb_philo)
	{
		if (pthread_create(&(check_deaths_thread[i]), NULL, &check_deaths, &philo))
		{
			free(philo_threads);
			free(check_deaths_thread);
			pthread_mutex_destroy(&(philo.mutex));
			j = 0;
			while (i < philo.nb_philo)
				pthread_mutex_destroy(&(philo.forks[j++]));
	...
	while (i < philo.nb_philo)
	{
		if (pthread_join(check_deaths_thread[i++], NULL))
		{
			free(philo_threads);
			free(check_deaths_thread);
			pthread_mutex_destroy(&(philo.mutex));
